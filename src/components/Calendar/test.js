...
function App() {
    ...
    const toggleCalendar = useCallback(
        (ev) => {
            ...
            if (checked) {
                ...
                setResources((resources) =>
                    resources.filter((item) => item.id !== calendarId)
                );
                setCalendarIds((calIds) =>
                    calIds.filter((item) => item !== calendarId)
                );
                setEvents((oldEvents) =>
                    oldEvents.filter(
                        (item) => item.outlookCalendarId !== calendarId
                    )
                );
            }
        },
    ...
    const handleUpdateConfirmClose = useCallback(
        (result) => {
            if (result) {
                ...
                setEvents((oldEvents) => [
                    ...oldEvents.filter((item) => item.id !== confirmEvent.id),
                    confirmOldEvent,
                ]);
            }
        ...
        const onSignedIn = () => {
            ...
            outlookCalendarSync
                ...
                .then((calendars) => {
                    ...
                    return outlookCalendarSync.getEvents(
                        newCalendarIds,
                        startDate.current,
                        endDate.current
                    );
    ...
    return (
        <Page
            className={
                "md-sync-events-outlook-cont " +
                (isLoading ? "md-loading-events" : "")
            }
        >
            <Eventcalendar
                view={myView}
            ...
            >
                <div className="mbsc-form-group-inset mbsc-align-center">
                    <p className="mbsc-italic mbsc-txt-muted">
                        Editing events sync back to your calendar when enabled.
                        You'll be asked for confirmation on every action.
                    </p>
                </div>
                ...
                <div className="mbsc-form-group-inset">
                    <Button
                        className="md-sync-events-outlook-button mbsc-button-block"
                        onClick={signOut}
                    >
